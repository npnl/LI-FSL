...
threshSet=( 2.3 1.0 1.5 ) #set the z-values here
	...
	do
			...
			for ROI in $ROIs; do
				...
				for cope in cope1 cope2; do #set for number of copes you want to calculate LI for
					...
					cluster -i zstat1.nii.gz --zthresh=$THRESH -o cluster_index --othresh=zstat1-$THRESH.nii #this sets the threshold and outputs the thresholded image to zstat1-$threshsSet.nii (e.g., zstat1-2.3.nii)            
					fslstats zstat1-$THRESH.nii.gz -k $ROI_dir/$ROI -V >> $ROOTDIR/$SUB/Laterality_Index/z-$THRESH/$ROI-$cope-voxels.txt #this takes the number of active voxels within the ROI and outputs it into a textfile labeled with the ROI name and cope number
...
for SUB in $SUBJS; do
	...
	#get val from each ROI, Left and Right
